...
def cl_forward(cls,
    ...
    pooler_outout = cls.pooler(attention_mask, outputs)
    pooler_output = pooler_outout.view(
        (batch_size, num_sent, pooler_outout.size(-1)))

    if cls.pooler_type == 'cls':
        pooler_outout = cls.mlp(pooler_outout)

    z1, z2 = pooler_outout[:, 0], pooler_outout[:, 1]
    # Hard negative
    if num_sent == 3:
        z3 = pooler_output[:, 2]
    ...
    if num_sent == 3:
        ...
        weights = torch.tensor(
            [[0.0] * (cos_sim.size(-1) - z1_z3_cos.size(-1)) + [0.0] * i +
             [z3_weight] + [0.0] * (z1_z3_cos.size(-1) - i - 1)
             for i in range(z1_z3_cos.size(-1))]).to(cls.device)
...
def sentemb_forward(cls,
    ...
    outputs = encoder(input_ids,
                      ...
                      inputs_embeds=inputs_embeds,
                      output_attentions=output_attentions,
                      output_hidden_states=True if cls.pooler_type
                      in ['avg_top2', 'avg_first_last'] else False,
                      return_dict=True)
...
class RobertaForCL(RobertaPreTrainedModel):
    __keys_to_ignore_on_load_missing = [r'position_ids']
