...
class LinformerSelfAttention(nn.Module):
    ...
    def forward(self, x, context=None, **kwargs):
        ...
        kv_len = n if context is not None else context.shape[1]
        assert (
            kv_len == self.seq_len
        ...
        kv_input = x if context is not None else context
        ...
        keys, values = map(merger_key_values, (keys, values))
...
class FeedForward(nn.Module):
    def __init__(self, dim, mult=4, dropout=0., activation=None, glu=False) -> None:
        super().__init__()
...
class LinformerLM(nn.Module):
    def __init__(self, num_tokens, dim, seq_len, depth, k=256, heads=8, dim_head=None, one_kv_head=False, share_kv=False, reversible=False, dropout=0.) -> None:
        super().__init__()
