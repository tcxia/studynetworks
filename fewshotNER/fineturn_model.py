...
class RobertaNER(RobertaForTokenClassification):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = loss_fct(
                logits.view(-1, self.dataset_label_nums[dataset]), labels.view(-1)
            )
            if output_logits:
    ...
    def forward_sup(
        ...
        log_pred_prob = torch.log(
            F.softmax(logits.view(-1, self.dataset_label_nums[dataset]), dim=-1)
        )
...
class BertNER(BertForTokenClassification):
    ...
    def __init__(self, config, dataset_label_nums, multi_gpus=False):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifiers = nn.ModuleList(
            [nn.Linear(config.hidden_size, x) for x in dataset_label_nums]
        )
        self.background = nn.Parameter(torch.zeros(1) - 2.0, requires_grad=True)
